...
def _atrous_conv2d(value, filters, rate, padding, name=None):
	return tf.nn.convolution(
		...
		filters=filters,
		padding=padding,
		dilations=np.broadcast_to(rate, (2,)),
	    data_format='NHWC',
	    name=name)
...
def conv2d(inputs, num_outputs, kernel_shape=[3, 3], strides=[1, 1], add_biases=True, pad='SAME', dilated=1, reuse=False, tower_index=None,
           W_init=tf.compat.v1.keras.initializers.VarianceScaling(scale=2.0, mode=('FAN_IN').lower(), distribution=("uniform" if False else "truncated_normal")), # msra
           b_init=tf.compat.v1.constant_initializer(0.0), W_params=None, b_params=None, wl=None, wl_type=tf.nn.l2_loss, summary=False, scope='conv2d'):
	"""
	Args:
	  inputs: NHWC
	...
	with tf.compat.v1.variable_scope(scope, reuse=reuse):
		...
		weights = tf.compat.v1.get_variable('w', weights_shape, tf.float32, W_init, use_resource=False)
		
		# add summary for w
		if summary and not reuse:
			tf.compat.v1.summary.histogram('hist_w', weights)
		...
		if dilated != 1:
			...
			outputs = tf.nn.conv2d(input=inputs, filters=weights, strides=[1, stride_h, stride_w, 1], padding=pad, data_format='NHWC')
		
		# add biases
		if add_biases:
			biases = tf.compat.v1.get_variable('b', [num_outputs], tf.float32, b_init, use_resource=False)
			
			# add summary for b
			if summary and not reuse:
				tf.compat.v1.summary.histogram('hist_b', biases)
		...
		if wl is not None:
			...
			tf.compat.v1.add_to_collection('losses_' + str(tower_index), weight_loss)
